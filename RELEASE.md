...
## Current verified release: v1.5.0

Novel Forge 1.5.0 is the pinned release for installation and supervised live-book pilots.
...
Use a copied or backed-up manuscript for the first pilot. Install the tag rather than an unpinned branch: `main` may contain unreleased work after the 1.5.0 release commit.

## 1.5.0 release record

- [x] `package.json`, `package-lock.json`, installed-version metadata, and new-project metadata report `1.5.0`.
- [x] `historical-fiction` is available through initialization, migration, organization, and next-book selection.
- [x] Historical context and invention ledgers are strict, conditional, guarded, hashed, and transactionally validated.
- [x] Research risk, chronology, knowledge, invention, affected-chapter, counterfactual-policy, and writer-decision rules validate together.
- [x] Drafting receives only a bounded Historical scene contract and excludes unrelated chronology and research.
- [x] Historical review lanes cover chronology, anachronism, impossible knowledge, provenance, portrayal, material plausibility, exposition, and voice.
- [x] Disclosed inventions derive a Historical Note through the existing package preview and approval flow.
- [x] Thriller and romantasy projects remain compatible and receive no historical-only files or checks.
...
- [x] Release notes preserve writer control, compatibility, automation, evidence, and verification boundaries.

The maintained release notes are in `docs/releases/v1.5.0.md`. Tags `v1.4.0`, `v1.4.1`, and `v1.4.2` remain immutable. Historical implementation plans under `docs/superpowers/` are records and are not current installation instructions.
