...
function temp(): string { return mkdtempSync(join(tmpdir(), "novel-forge-v13-version-")); }

test("new projects use the 1.3 contract and retain canonical 1.2 metadata files", () => {
  const parent = temp();
  try {
    const root = initializeProject(parent, { projectName: "V13", projectType: "standalone", profile: "thriller" });
    assert.equal(NOVEL_FORGE_VERSION, "1.3.0");
...
test("1.2 projects warn while newer projects block", () => {
  ...
  try {
    ...
    assert.ok(!versionFindings(project12).some((finding) => finding.severity === "blocker"));
    const project14 = { ...project, novel_forge_version: "1.4.0" };
    assert.ok(versionFindings(project14).some((finding) => finding.severity === "blocker" && /newer/i.test(finding.message)));
  } finally { rmSync(parent, { recursive: true, force: true }); }
});
