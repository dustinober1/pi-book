...
function capsule(): ActiveContextCapsule {
  return {
    ...
    capsule_id: "CAP-7777777777777777",
    ...
    scene_contract: {
      ...
      turn: "Mara uses the maintenance route.",
      required_beats: ["Reach the terminal", "Record Mara at the terminal"],
      active_thread_ids: [],
      ...
      target_words: { minimum: 20, maximum: 120 },
    ...
    records: [{
      ...
      required: true,
      reason: "expected state delta",
      source_path: "series/state-ledger.yaml",
      source_hash: "c".repeat(64),
      version: 1,
      payload: { subject_id: "CHAR-MARA", field: "location", value: "LOC-CORRIDOR" },
      dependencies: [],
      estimated_tokens: 20,
    }],
    previous_tail: null,
...
function validation(value: SceneDraftArtifact): SceneValidationArtifact {
  return {
    ...
    scene_id: sceneId,
    draft_attempt: 1,
    draft_output_hash: value.output_hash,
...
function delta(value: SceneDraftArtifact): SceneStateDeltaArtifact {
  return {
    ...
    mismatches: [{
      ...
      field: "location",
      message: "Scene does not establish expected mutation STATE-MARA-LOCATION.location.",
    }],
    matches_expected: false,
    next_action: "span-repair",
    usage: {
      callId: "delta-1",
      stage: "drafting",
      chapter: 1,
  ...
  async run(request: QualityWorkerRequest): Promise<QualityWorkerResult> {
    ...
    return {
      text: JSON.stringify({
        ...
        operations: [{
          operation: "replace",
          anchor_quote: "Mara reached the terminal",
