...
test("Novel Forge 1.10.0 release assets exist and package metadata is aligned", () => {
  const checks = verifyV1100ReleaseTree(root);
...
test("release docs still describe authority, budgets, privacy, and evaluation limits", () => {
  const joined = ["README.md", "SKILL.md", "CHANGELOG.md", "RELEASE.md", "docs/quality-and-cost.md", "docs/releases/v1.10.0.md"]
    .map((path) => readFileSync(join(root, path), "utf8"))
    .join("\n");
  for (const phrase of ["1.10.0", "economy", "balanced", "premium", "editorial", "guarded", "budget", "telemetry", "paid evaluation", "human review"]) {
