...
test("the historical v1.5 release checker rejects current v1.9.1 metadata", () => {
  const root = process.cwd();
  const packageJson = JSON.parse(readFileSync(join(root, "package.json"), "utf8"));
  const lock = JSON.parse(readFileSync(join(root, "package-lock.json"), "utf8"));
  assert.equal(packageJson.version, "1.9.1");
  assert.equal(lock.version, "1.9.1");
  assert.equal(lock.packages[""].version, "1.9.1");
  assert.equal(NOVEL_FORGE_VERSION, "1.9.1");
