...
import { verifyV220ReleaseTree } from "../scripts/verify-v2-2-0-release.js";

const root = process.cwd();

test("Novel Forge 2.2.0 release assets exist and package metadata is aligned", () => {
  const checks = verifyV220ReleaseTree(root);
...
test("release docs still describe authority, budgets, privacy, and evaluation limits", () => {
  const joined = ["README.md", "SKILL.md", "CHANGELOG.md", "RELEASE.md", "docs/quality-and-cost.md", "docs/releases/v2.2.0.md"]
    .map((path) => readFileSync(join(root, path), "utf8"))
    .join("\n");
  for (const phrase of ["2.2.0", "economy", "balanced", "premium", "editorial", "guarded", "budget", "telemetry", "paid evaluation", "human review"]) {
...
test("every part of the release set reports the same version", () => {
  ...
  assert.match(readFileSync(join(root, "src/application/version-core.ts"), "utf8"), /NOVEL_FORGE_VERSION\s*=\s*"2\.2\.0"/);
  assert.equal(packageJson.scripts["verify:release"], "node --import tsx scripts/verify-v2-2-0-release.ts");
  assert.match(packageJson.scripts["test:release"] ?? "", /tests\/v2-2-0-release-checklist\.test\.ts/);
  assert.equal(existsSync(join(root, "docs/releases/v2.2.0.md")), true);
  assert.match(readFileSync(join(root, "README.md"), "utf8"), /@v2\.2\.0/);
  assert.match(readFileSync(join(root, "RELEASE.md"), "utf8"), /## Current verified release: v2\.2\.0/);
});

// The whole point of this release: six "## Unreleased" sections describing
// shipped-but-unreleased work must not still be sitting above the version
...
test("the changelog carries no unreleased sections", () => {
  ...
  assert.match(changelog, /## 2\.2\.0 — One Run to a Packaged Book on a Local Model/);
});
