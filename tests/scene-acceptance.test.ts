...
const sceneContractHashes = { [scenes[0]]: "c".repeat(64), [scenes[1]]: "d".repeat(64) };
const storyIndexHash = "b".repeat(64);
const hash = (value: string) => createHash("sha256").update(value, "utf8").digest("hex");
const sceneHash = (sceneId: typeof scenes[number]) => sceneContractHashes[sceneId];

function prose(sceneId: string): string {
  return sceneId === scenes[0] ? "Mara crossed the corridor and reached the first terminal." : "Mara copied the final access log and sealed the terminal behind her.";
}

function draft(sceneId: typeof scenes[number]): SceneDraftArtifact {
  ...
  return {
    ...
    capsule_id: "CAP-0123456789ABCDEF", contract_hash: sceneHash(sceneId), story_index_hash: storyIndexHash,
    ...
    usage: { callId: `${sceneId}-draft`, stage: "drafting", chapter: 1, sceneId, attempt: 1, pass: "candidate", jobType: "draft-scene", contractHash: sceneHash(sceneId), capsuleHash: "e".repeat(64), includedRecordCount: 0, estimated: true, elapsedMs: 1, promptHash: "f".repeat(64), contextHash: "1".repeat(64), outputHash },
    created_at: "2026-07-22T00:00:00.000Z",
  };
}

function validation(value: SceneDraftArtifact): SceneValidationArtifact {
  return { schema_version: "1.0.0", run_id: value.run_id, chapter: 1, scene_id: value.scene_id, draft_attempt: 1, draft_output_hash: value.output_hash, capsule_id: value.capsule_id, contract_hash: value.contract_hash, findings: [], blocker_count: 0, warning_count: 0, passed: true, next_node: "critic-review", created_at: "2026-07-22T00:00:01.000Z" };
}
function critics(value: SceneDraftArtifact): SceneCriticSummaryArtifact {
  return { schema_version: "1.0.0", run_id: value.run_id, chapter: 1, scene_id: value.scene_id, draft_attempt: 1, draft_output_hash: value.output_hash, contract_hash: value.contract_hash, required_job_types: ["critic-continuity"], critics: [{ job_type: "critic-continuity", critic_attempt: 1, verdict: "pass", finding_count: 0 }], blocker_count: 0, repair_count: 0, passed: true, next_action: "state-delta", created_at: "2026-07-22T00:00:02.000Z" };
}
...
function setup(sceneId: typeof scenes[number], accepted: string[] = [], matchesExpected = true) {
  ...
  let state = createChapterExecutionState({ runId: candidate.run_id, projectHash: projectStateHash(root), canonSnapshotHash: storyIndexHash, contractHash: sceneHash(sceneId), chapterContractHash, chapter: 1 });
  for (const node of ["scene-contract-compile", "context-build", "scene-plan", "scene-draft", "deterministic-validation", "critic-review", "state-delta", "scene-accept"] as const) state = transitionChapterExecution(state, node, undefined, node === "scene-contract-compile" ? undefined : sceneId);
...
test("mismatched delta, skipped prior scene, or missing next-scene hash blocks acceptance", () => {
  const mismatch = setup(scenes[0], [], false);
  try {
    assert.throws(() => acceptSceneCandidate(acceptanceInput(mismatch.root, mismatch.runId, scenes[0])), /state delta.*match|not ready|scene-accept/i);
