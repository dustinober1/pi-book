...
test("starting and pausing a run persists intent without changing creative state", () => {
  ...
  try {
    ...
    assert.equal(readProject(root).automation.active_run?.status, "paused");
...
test("outside creative changes stop resume before another prompt", () => {
  ...
  try {
    ...
    assert.equal(readProject(root).automation.active_run?.status, "stopped");
    assert.match(readProject(root).automation.active_run?.stopReason ?? "", /changed|hash|replan/i);
  } finally { rmSync(parent, { recursive: true, force: true }); }
});
...
test("cancel is persisted and cannot resume", () => {
  ...
  try {
    ...
    assert.equal(readProject(root).automation.active_run?.status, "cancelled");
