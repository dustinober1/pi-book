...
test("Novel Forge 1.7.1 release assets exist and package metadata is aligned", () => {
  const checks = verifyV171ReleaseTree(root);
...
test("release docs still describe authority, budgets, privacy, and evaluation limits", () => {
  const joined = ["README.md", "SKILL.md", "CHANGELOG.md", "RELEASE.md", "docs/quality-and-cost.md", "docs/releases/v1.7.1.md"]
    .map((path) => readFileSync(join(root, path), "utf8"))
    .join("\n");
  for (const phrase of ["1.7.1", "economy", "balanced", "premium", "editorial", "guarded", "budget", "telemetry", "paid evaluation", "human review"]) {
...
test("the release workflow is a single file shared across versions", () => {
  ...
  for (const retired of [
    ...
    assert.equal(existsSync(join(root, ".github/workflows", retired)), false, retired);
  }
});
