...
test("preparation compiles deterministic scene hashes and persists dual contract ownership", () => {
  ...
  try {
    ...
    assert.equal(result.manifest.model_execution_profile, "host-default");
...
test("exact Gemma preparation rejects a new manifest without a qualified fingerprint", () => {
  const { parent, root } = setup();
  try {
    ...
    assert.throws(
      () => prepareChapterExecution({ root, chapter: 1, runId: "RUN-GEMMA-UNQUALIFIED" }),
      /qualified Gemma fingerprint/i,
    );
    assert.equal(readChapterExecutionManifest(root, "RUN-GEMMA-UNQUALIFIED", 1), null);
    assert.equal(readChapterExecutionState(root, "RUN-GEMMA-UNQUALIFIED"), null);
  } finally { rmSync(parent, { recursive: true, force: true }); }
});
