...
const genreSettingsSchema = Type.Object({
  thriller_type: Type.Union([Type.Literal("techno"), Type.Literal("military-intelligence"), Type.Literal("political"), Type.Literal("psychological"), Type.Literal("legal"), Type.Literal("medical"), Type.Literal("espionage"), Type.Literal("crime"), Type.Literal("domestic"), Type.Literal("action"), Type.Literal("conspiracy")]),
...
function packetFieldFindings(packet: ChapterPacket): ProfileFinding[] { return requiredProfileFields.flatMap((field) => typeof packet.profile_fields[field] === "string" && String(packet.profile_fields[field]).trim() ? [] : [{ severity: "blocker" as const, category: "thriller-packet", message: `Missing or invalid thriller packet field: ${field}` }]); }
export function thrillerArchitectureFindings(plot: PlotGridState): ProfileFinding[] {
  if (plot.chapters.length < 3) return [];
  const obligations = plot.chapters.flatMap((chapter) => chapter.profile_obligations);
  const findings: ProfileFinding[] = [];
  if (!obligations.some((item) => /deadline|clock|window|before .* happen|consequence of delay/i.test(item))) findings.push({ severity: "high", category: "external-clock", message: "Thriller plan lacks a concrete external clock or stated consequence of delay." });
  if (!obligations.some((item) => /objective|deliver|stop|prevent|escape|expose|protect|recover|reach/i.test(item))) findings.push({ severity: "high", category: "act-objective", message: "Thriller plan lacks a concrete protagonist objective for operational scenes." });
  if (!obligations.some((item) => /opposition|benefit|incentive|stake|value|why .* wants|positive case/i.test(item))) findings.push({ severity: "high", category: "opposition-benefit", message: "Thriller plan does not state the opposition's concrete benefit or defensible value." });
  if (!obligations.some((item) => /counterevidence|partial truth|wrong|mistake|costly|compromise|moral/i.test(item))) findings.push({ severity: "high", category: "counterevidence", message: "Thriller plan does not seed counterevidence or a morally complicating cost for the protagonist." });
  return findings;
}
