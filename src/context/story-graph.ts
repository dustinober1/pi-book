...
export type StoryGraphBlockReason = "provisional" | "future-book" | "inactive-thread" | "unready-research";
...
export interface StoryGraphNode {
  ...
  introducedIn?: string | null | undefined;
}
...
export interface StoryGraphInput {
  ...
  research?: ResearchLedger | undefined;
}
...
function characterNodeId(name: string): string { return `character:${normalizeEntity(name)}`; }
function entityNodeId(subject: string): string { return `entity:${normalizeEntity(subject)}`; }
function chapterNodeId(chapter: number): string { return `chapter:${chapter}`; }
function canonFactNodeId(id: string): string { return `canon-fact:${id}`; }
function relationshipNodeId(id: string): string { return `relationship:${id}`; }
function storyThreadNodeId(id: string): string { return `story-thread:${id}`; }
function researchItemNodeId(id: string): string { return `research-item:${id}`; }
function researchSourceNodeId(id: string): string { return `research-source:${id}`; }
function storyElementNodeId(id: string): string { return `story-element:${id}`; }
...
function addEdge(graph: MutableGraph, from: string, to: string, edgeType: StoryGraphEdgeType, provenance: string): void {
...
function ensureStoryElementNode(graph: MutableGraph, bookId: string, refId: string): string {
  ...
  addNode(graph, { id, type: "story-element", refId, label: refId, sourcePath: `books/${bookId}/plot-grid.yaml#${refId}` });
  return id;
}

function sortGraph(graph: MutableGraph): StoryGraph {
  for (const neighbors of graph.adjacency.values()) {
    neighbors.sort((a, b) => a.nodeId.localeCompare(b.nodeId) || a.edgeType.localeCompare(b.edgeType) || a.provenance.localeCompare(b.provenance));
  }
  return { bookId: graph.bookId, nodes: graph.nodes, adjacency: graph.adjacency, referenceIndex: graph.referenceIndex };
}
...
export function buildStoryGraph(input: StoryGraphInput): StoryGraph {
  ...
  for (const packet of input.queue.packets) for (const character of packet.character_refs) rememberCharacter(character);
  for (const relationship of input.canon.relationships) for (const character of relationship.characters) rememberCharacter(character);
  for (const thread of input.threads.threads) for (const character of Object.keys(thread.characters_know)) rememberCharacter(character);
  ...
  for (const [normalized, label] of [...knownCharacters.entries()].sort(([a], [b]) => a.localeCompare(b))) {
    addNode(graph, { id: `character:${normalized}`, type: "character", refId: label, label, sourcePath: "derived:known-character" });
  }
  for (const fact of input.canon.facts) {
    addNode(graph, { id: canonFactNodeId(fact.id), type: "canon-fact", refId: fact.id, label: fact.fact, sourcePath: `series/canon.yaml#facts.${fact.id}`, status: fact.status, introducedIn: fact.introduced_in });
  }
  for (const relationship of input.canon.relationships) {
    addNode(graph, { id: relationshipNodeId(relationship.id), type: "relationship", refId: relationship.id, label: relationship.state, sourcePath: `series/canon.yaml#relationships.${relationship.id}`, status: relationship.status });
  }
  for (const thread of input.threads.threads) {
    addNode(graph, { id: storyThreadNodeId(thread.id), type: "story-thread", refId: thread.id, label: thread.setup, sourcePath: `series/story-threads.yaml#threads.${thread.id}`, status: thread.status });
  }
  for (const item of input.research?.items ?? []) {
    addNode(graph, { id: researchItemNodeId(item.id), type: "research-item", refId: item.id, label: item.claim || item.notes || item.id, sourcePath: `books/${input.bookId}/research-ledger.yaml#items.${item.id}`, status: item.status });
  }
  for (const source of input.sources.sources) {
    addNode(graph, { id: researchSourceNodeId(source.id), type: "research-source", refId: source.id, label: source.title, sourcePath: `research/source-register.yaml#sources.${source.id}` });
  }

  for (const fact of input.canon.facts) {
    const subjectKey = normalizeEntity(fact.subject);
    const subjectNode = knownCharacters.has(subjectKey) ? characterNodeId(fact.subject) : entityNodeId(fact.subject);
    addNode(graph, { id: subjectNode, type: knownCharacters.has(subjectKey) ? "character" : "entity", refId: fact.subject, label: fact.subject, sourcePath: knownCharacters.has(subjectKey) ? "derived:known-character" : "derived:canon-subject" });
    ...
    if (sourceChapter !== null) addEdge(graph, canonFactNodeId(fact.id), ensureChapterNode(graph, input.bookId, sourceChapter), "sourced-from", `series/canon.yaml#facts.${fact.id}.source`);
  }

  for (const relationship of input.canon.relationships) {
    for (const character of relationship.characters) {
      const id = characterNodeId(character);
      addNode(graph, { id, type: "character", refId: character, label: character, sourcePath: "derived:known-character" });
      addEdge(graph, relationshipNodeId(relationship.id), id, "involves", `series/canon.yaml#relationships.${relationship.id}.characters`);
    }
  }
  ...
  for (const thread of input.threads.threads) {
    for (const character of Object.keys(thread.characters_know)) {
      const id = characterNodeId(character);
      addNode(graph, { id, type: "character", refId: character, label: character, sourcePath: "derived:known-character" });
      addEdge(graph, storyThreadNodeId(thread.id), id, "known-by", `series/story-threads.yaml#threads.${thread.id}.characters_know`);
    }
    const advancedChapter = chapterNumber(thread.last_advanced_in);
    if (advancedChapter !== null) addEdge(graph, storyThreadNodeId(thread.id), ensureChapterNode(graph, input.bookId, advancedChapter), "last-advanced-in", `series/story-threads.yaml#threads.${thread.id}.last_advanced_in`);
  }

  for (const packet of input.queue.packets) {
    const chapterId = ensureChapterNode(graph, input.bookId, packet.chapter);
    for (const character of packet.character_refs) addEdge(graph, chapterId, characterNodeId(character), "features-character", `books/${input.bookId}/chapter-queue.yaml#chapter-${packet.chapter}.character_refs`);
    for (const ref of packet.continuity_refs) {
      for (const target of [canonFactNodeId(ref), relationshipNodeId(ref)]) {
        if (graph.nodes.has(target)) addEdge(graph, chapterId, target, "references", `books/${input.bookId}/chapter-queue.yaml#chapter-${packet.chapter}.continuity_refs`);
      }
    }
    for (const ref of packet.story_thread_refs) {
      const target = storyThreadNodeId(ref);
      if (graph.nodes.has(target)) addEdge(graph, chapterId, target, "references", `books/${input.bookId}/chapter-queue.yaml#chapter-${packet.chapter}.story_thread_refs`);
    }
    for (const ref of packet.required_research) {
      const researchTarget = researchItemNodeId(ref);
      const sourceTarget = researchSourceNodeId(ref);
      if (graph.nodes.has(researchTarget)) addEdge(graph, chapterId, researchTarget, "requires-research", `books/${input.bookId}/chapter-queue.yaml#chapter-${packet.chapter}.required_research`);
      else if (graph.nodes.has(sourceTarget)) addEdge(graph, chapterId, sourceTarget, "requires-source", `books/${input.bookId}/chapter-queue.yaml#chapter-${packet.chapter}.required_research`);
    }
  }
  ...
  for (const entry of input.plot.chapters) {
    ...
    for (const ref of entry.setup_ids) {
      ...
      for (const target of targets.length ? targets : [ensureStoryElementNode(graph, input.bookId, ref)]) addEdge(graph, chapterId, target, "sets-up", `books/${input.bookId}/plot-grid.yaml#chapter-${entry.chapter}.setup_ids`);
    }
    for (const ref of entry.payoff_ids) {
      const targets = knownReferenceNodeIds(graph, ref);
      for (const target of targets.length ? targets : [ensureStoryElementNode(graph, input.bookId, ref)]) addEdge(graph, chapterId, target, "pays-off", `books/${input.bookId}/plot-grid.yaml#chapter-${entry.chapter}.payoff_ids`);
    }
  }

  const sourceById = new Map(input.sources.sources.map((source) => [source.id, source]));
  for (const item of input.research?.items ?? []) {
    for (const sourceId of item.source_ids) {
      const source = sourceById.get(sourceId);
      if (!source?.supports_research_ids?.includes(item.id)) continue;
      addEdge(graph, researchItemNodeId(item.id), researchSourceNodeId(sourceId), "sourced-from", `books/${input.bookId}/research-ledger.yaml#items.${item.id}.source_ids`);
    }
  }
  ...
  for (const source of input.sources.sources) {
    ...
    for (const ref of source.supports) {
      for (const target of knownReferenceNodeIds(graph, ref)) {
        if (target !== sourceId) addEdge(graph, sourceId, target, "supports", `research/source-register.yaml#sources.${source.id}.supports`);
      }
    }
  }
...
function blockReason(node: StoryGraphNode, activeBookId: string): StoryGraphBlockReason | null {
  if (node.type === "canon-fact") {
    ...
    return introduced !== null && active !== null && introduced > active ? "future-book" : null;
  }
  ...
  if (node.type === "research-item") return node.status === "ready" ? null : "unready-research";
  return null;
}
...
function selectionFor(node: StoryGraphNode, visit: Visit, reason: StoryGraphSelection["reason"]): StoryGraphSelection {
  ...
  return { nodeId: node.id, refId: node.refId, type: node.type, depth: visit.depth, reason, path: visit.path, sourcePath: node.sourcePath };
}

function blockedSelectionFor(node: StoryGraphNode, visit: Visit, reason: StoryGraphBlockReason): StoryGraphBlockedSelection {
  if (node.type === "research-source" || !isRecordNode(node)) throw new Error(`Cannot block unsupported graph node ${node.id}.`);
  return { nodeId: node.id, refId: node.refId, type: node.type, depth: visit.depth, reason, path: visit.path, sourcePath: node.sourcePath };
}

function compareSelections(a: StoryGraphSelection, b: StoryGraphSelection): number {
  return (a.reason === "explicit" ? 0 : 1) - (b.reason === "explicit" ? 0 : 1)
    || a.depth - b.depth
...
function compareBlocked(a: StoryGraphBlockedSelection, b: StoryGraphBlockedSelection): number {
  return a.depth - b.depth || TYPE_ORDER[a.type] - TYPE_ORDER[b.type] || a.refId.localeCompare(b.refId) || a.nodeId.localeCompare(b.nodeId);
}

function explicitSeedNodeIds(graph: StoryGraph, packet: ChapterPacket): Set<string> {
  const ids = new Set<string>();
  for (const ref of packet.continuity_refs) for (const candidate of [canonFactNodeId(ref), relationshipNodeId(ref)]) if (graph.nodes.has(candidate)) ids.add(candidate);
  for (const ref of packet.story_thread_refs) {
    const candidate = storyThreadNodeId(ref);
    if (graph.nodes.has(candidate)) ids.add(candidate);
  }
  for (const ref of packet.required_research) {
    const researchCandidate = researchItemNodeId(ref);
    const sourceCandidate = researchSourceNodeId(ref);
    if (graph.nodes.has(researchCandidate)) ids.add(researchCandidate);
    else if (graph.nodes.has(sourceCandidate)) ids.add(sourceCandidate);
  }
  return ids;
}

export function resolveDraftingGraphContext(graph: StoryGraph, packet: ChapterPacket, options?: { maxDepth?: 1 | 2 }): StoryGraphResolution {
  ...
  while (cursor < queue.length) {
    ...
    for (const neighbor of graph.adjacency.get(current.id) ?? []) {
      ...
      if (!node) continue;
      const next: Visit = { nodeId: node.id, depth: visit.depth + 1, path: [...visit.path, node.id] };
  ...
  const idsByType = (type: StoryGraphRecordNodeType): string[] => orderedSelections.filter((selection) => selection.type === type).map((selection) => selection.refId);
  return {
    maxDepth,
    factIds: idsByType("canon-fact"),
    relationshipIds: idsByType("relationship"),
    threadIds: idsByType("story-thread"),
    researchIds: idsByType("research-item"),
