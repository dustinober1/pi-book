 ...
 * model supplies only the two decisions that are actually its own, as typed
 * values, and the tool serialises and applies them through the same guarded
...
export function registerCompleteContractCommand(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    description: "Complete a compiled chapter contract from typed values and apply it through one guarded chapter-queue event. Start states and knowledge boundaries are derived from the story ledgers; you supply only what this chapter must change and must not change.",
    promptSnippet: "Complete the chapter contract by naming what the chapter must change and must not change.",
    promptGuidelines: [
