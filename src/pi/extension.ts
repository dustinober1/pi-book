...
import { bookPlanPrompt, packagePrompt, readerTestPrompt, reviewPrompt, seriesPlanPrompt, voiceAuditPrompt, voicePlanPrompt } from "../application/prompts.js";
...
import { voiceAuditRequirementForScope } from "../application/voice-drift.js";
...
export function registerNovelForge(pi: ExtensionAPI): void {
  ...
  pi.registerCommand("novel-review", { description: "Review a chapter, act, manuscript, series, or explicit voice recalibration", getArgumentCompletions: (prefix) => { const filtered = ["chapter", "act", "manuscript", "series", "recalibration"].filter((item) => item.startsWith(prefix)).map((value) => ({ value, label: value })); return filtered.length ? filtered : null; }, handler: async (args, context) => { try { const root = requireProjectRoot(context.cwd); const scope = tokens(args)[0] ?? "act"; assertReviewAllowed(readProject(root), scope); if (scope === "recalibration") { const requirement = voiceAuditRequirementForScope(root, scope); if (!requirement) throw new Error("Voice recalibration requires a Phase 5 voice-audits.yaml artifact."); sendDecision(pi, context, { action: "voice-audit", prompt: voiceAuditPrompt(root, requirement), message: `Queued ${requirement.milestone_ref}.` }); return; } sendDecision(pi, context, { action: "review", prompt: reviewPrompt(root, scope), message: `Queued ${scope} review.` }); } catch (error) { context.ui.notify(errorText(error), "warning"); } } });
