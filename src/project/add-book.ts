...
import { readBook, readProject } from "./store.js";
import { bookTemplateFiles } from "./templates.js";
import { applyGuidedProjectEvent } from "../application/handoff.js";
...
export function addBook(root: string, targetWords = 100000, options: AddBookOptions = {}): string {
  ...
  const existing = existsSync(booksRoot)
    ? readdirSync(booksRoot, { withFileTypes: true }).filter((entry) => entry.isDirectory() && /^book-\d{2}$/.test(entry.name)).map((entry) => entry.name).sort()
    : [];
  const nextNumber = existing.length + 1;
  if (nextNumber > 99) throw new Error("Novel Forge supports up to 99 books in one workspace.");
  const bookId = `book-${String(nextNumber).padStart(2, "0")}`;
  if (existsSync(join(booksRoot, bookId))) throw new Error(`${bookId} already exists.`);
  ...
  const arcPath = join(root, "series", "series-arc.yaml");
  const arcText = readText(arcPath) ?? 'schema_version: "1.0.0"\nbooks: []\nlong_arcs: []\n';
  const arc = parseYaml<SeriesArcState>(arcText, SeriesArcSchema, "series-arc.yaml");
  const previous = arc.books.find((item) => item.id === currentBook.book_id);
  if (previous) previous.status = currentBook.canon_locked ? "locked" : "superseded-by-force";
  ...
  const changes = [
    ...
    ...Object.entries(bookTemplateFiles(bookId, nextNumber, project.default_profile, targetWords)).map(([path, content]) => ({ path, content })),
  ];
  applyGuidedProjectEvent(root, changes, `Novel Forge: add ${bookId}`, { lastAction: `Created ${bookId}` });
  mkdirSync(join(root, "books", bookId, "manuscript", "chapters"), { recursive: true });
  return bookId;
}