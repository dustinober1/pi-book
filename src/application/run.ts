...
export function approveProjectGate(root: string, gate: string): RunDecision {
  const project = approveGate(readProject(root), gate);
  writeProjectEvent(root, [{ path: "PROJECT.yaml", content: stringifyYaml(project) }], `Novel Forge: approve ${gate}`);
...
export function decideNextRun(root: string, options: RunOptions = {}): RunDecision {
  ...
  if (options.reviewOnly) return { action: "review", prompt: reviewPrompt(root, "current milestone"), message: "Queued milestone review." };
  switch (project.current_stage) {
    ...
    case "drafting": {
      ...
      return {
        action: "bounded-draft",
        prompt: automationDraftPrompt(root, maxChapters, options.until),
        message: `Queued a bounded drafting run of up to ${maxChapters} chapter(s).`,
      };
    }
    case "act-review": return { action: "review", prompt: reviewPrompt(root, options.until ?? "act"), message: "Queued act review." };
    case "revision": {
