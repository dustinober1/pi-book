...
function requireState(input: ValidateSceneDraftInput): ChapterExecutionState {
  ...
  if (input.capsule.project_hash !== state.project_hash) throw new Error("Cannot validate scene because the capsule project hash does not match the active project snapshot.");
