...
import { ChapterValidationArtifactSchema, type ChapterValidationArtifact, type ChapterValidationFinding, type ChapterValidationFindingCode } from "../domain/chapter-validation-artifact.js";
...
import { blockChapterExecution, chapterContractHash, transitionChapterExecution } from "./chapter-execution-machine.js";
import { projectStateHash } from "./project-hash.js";

export interface ValidateStitchedChapterInput { root: string; runId: string; chapter: number; now?: string; }
export interface ValidateStitchedChapterResult { artifact: ChapterValidationArtifact; artifactPath: string; state: ChapterExecutionState; }
...
function finding(code: ChapterValidationFindingCode, message: string, severity: "blocker" | "warning" = "blocker"): ChapterValidationFinding { return { code, severity, message }; }
...
function requireStitch(input: ValidateStitchedChapterInput, state: ChapterExecutionState): ChapterStitchArtifact {
  ...
  if (stitch.run_id !== input.runId || stitch.chapter !== input.chapter || stitch.contract_hash !== chapterContractHash(state) || stitch.story_index_hash !== state.canon_snapshot_hash || stitch.next_node !== "chapter-validate") throw new Error("Chapter stitch provenance does not match the chapter contract validation checkpoint.");
  ...
  if (stitch.scene_ids.length !== stitch.scenes.length || stitch.scene_ids.some((sceneId, index) => stitch.scenes[index]?.scene_id !== sceneId)) throw new Error("Chapter stitch scene-order provenance is inconsistent.");
  if (state.accepted_scene_ids.length !== stitch.scene_ids.length || stitch.scene_ids.some((sceneId, index) => state.accepted_scene_ids[index] !== sceneId)) throw new Error("Chapter stitch scene order does not match accepted execution scenes.");
  return stitch;
}
...
function requireAcceptances(input: ValidateStitchedChapterInput, stitch: ChapterStitchArtifact): SceneAcceptanceArtifact[] {
  return stitch.scenes.map((scene, index) => {
    ...
    if (acceptance.run_id !== input.runId || acceptance.chapter !== input.chapter || acceptance.scene_id !== scene.scene_id || acceptance.draft_attempt !== scene.draft_attempt || acceptance.draft_output_hash !== scene.draft_output_hash || acceptance.word_count !== scene.word_count || acceptance.contract_hash !== scene.contract_hash || acceptance.story_index_hash !== stitch.story_index_hash) throw new Error(`Scene acceptance provenance or scene contract does not match stitched chapter for ${scene.scene_id}.`);
    if (artifactHash(acceptance) !== scene.acceptance_artifact_hash) throw new Error(`Scene acceptance artifact hash provenance failed for ${scene.scene_id}.`);
    const expectedNext = stitch.scene_ids[index + 1] ?? null;
    const expectedNode = expectedNext ? "context-build" : "chapter-stitch";
    if (acceptance.next_scene_id !== expectedNext || acceptance.next_node !== expectedNode) throw new Error(`Scene acceptance routing provenance is inconsistent for ${scene.scene_id}.`);
    return acceptance;
  });
}
...
function contentFindings(stitch: ChapterStitchArtifact): ChapterValidationFinding[] {
  ...
  if (/\b(?:here is|here's)\s+(?:the\s+)?(?:chapter|scene|draft)\b|\bas an ai\b|^\s*analysis\s*:/im.test(text)) findings.push(finding("meta-commentary", "Stitched chapter contains assistant or drafting commentary."));
...
export function validateStitchedChapter(input: ValidateStitchedChapterInput): ValidateStitchedChapterResult {
  ...
  const artifact: ChapterValidationArtifact = { schema_version: "1.0.0", run_id: input.runId, chapter: input.chapter, stitch_artifact_hash: artifactHash(stitch), stitch_output_hash: stitch.output_hash, contract_hash: stitch.contract_hash, story_index_hash: stitch.story_index_hash, scene_ids: [...stitch.scene_ids], findings, blocker_count: blockerCount, warning_count: warningCount, passed, next_action: passed ? "chapter-commit" : "blocked", created_at: timestamp(input.now) };
  ...
  const advanced = passed ? transitionChapterExecution(state, "chapter-commit", input.now, state.current_scene_id ?? undefined) : blockChapterExecution(state, { code: "schema-failure", message: `Chapter ${input.chapter} failed deterministic validation.`, recordIds: [...new Set(findings.filter((item) => item.severity === "blocker").map((item) => item.code))] }, input.now);
