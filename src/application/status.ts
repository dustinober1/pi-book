...
import { ContinuityDeltaSchema, type ContinuityDeltaState } from "../domain/schemas.js";
...
function nextActionForStage(stage: string): string {
  const actions: Record<string, string> = {
    ...
    drafting: "Run /novel-draft or /novel-run.",
    "act-review": "Run /novel-review act.",
    revision: "Run /novel-revise.",
    "manuscript-review": "Run /novel-review manuscript.",
...
export function getProjectStatus(root: string): ProjectStatus {
  ...
  const deltaText = readText(join(root, "books", book.book_id, "continuity-delta.yaml"));
  if (deltaText) {
  ...
  const required = [
    "series/series-bible.md", "series/voice-profile.md", "series/series-arc.yaml", "series/canon.yaml", "series/story-threads.yaml",
    `books/${book.book_id}/book-bible.md`, `books/${book.book_id}/genre.yaml`, `books/${book.book_id}/plot-grid.yaml`,
    `books/${book.book_id}/chapter-queue.yaml`, `books/${book.book_id}/revision-tickets.yaml`,
  ];
  ...
  const markdown = [
    ...
    "", "## Recent files", "", ...(recent.length ? recent.map((item) => `- ${item.path}`) : ["- none"]), "",
  ].join("\n");
