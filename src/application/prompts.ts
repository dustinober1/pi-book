...
export function voicePlanPrompt(root: string): string {
  return `Use the novel-forge-for-pi skill.\n\nProject root: ${root}\n\n${interviewRule}\n\nFor voice intake, resolve intended reader effect, positive evidence, unwanted tendencies, productive imperfections, and lived-material boundaries. Build series/voice-profile.md from the writer's own samples, accepted chapters, stated preferences, and not-this-author examples. Translate named author or book references into high-level craft traits rather than imitating exact style. Preserve productive roughness and never invent lived experience, taste evidence, samples, or an accepted baseline.\n\nPrepare only series/voice-profile.md, series/taste-profile.yaml, and series/voice-guardrails.yaml. Keep unknown evidence empty and preserve the fixed writer-first precedence in taste-profile.yaml.\n\n${eventRule(root, "voice-profile", "voice-intake")}`;
}
...
export function bookPlanPrompt(root: string): string {
  ...
  return `Use the novel-forge-for-pi skill.\n\nProject root: ${root}\nActive book: ${book.book_id}\n\n${interviewRule}\n\nThe four primary author decisions are:\n1. What is the safe, predictable version of this book that must be avoided?\n2. What can this project uniquely deliver?\n3. What moment should readers retell to someone else?\n4. What should remain alive after the ending?\n\nUse those answers plus existing evidence to derive the complete typed genre, plot, queue, continuity, thread, remarkability, research, and strategy artifacts. The finished plan must still define the book promise, POV rules, conflicts, opposition, ending contract, research dependencies, acts, chapter causality, setup/payoff IDs, profile obligations, productive discomfort, 2–5 signature moments, productive disagreements, restrained motifs, a hand-sell reason, accepted reader costs, and a reader-facing strategy. Do not make the writer fill out the schema field by field. Do not invent research claims, public reviews, reader reactions, or approved guardrails.\n\nProfile planning questions to answer from evidence or the short interview:\n${profile.planningQuestions.map((item) => `- ${item}`).join("\n")}\n\nPrepare only books/${book.book_id}/book-bible.md, genre.yaml, plot-grid.yaml, chapter-queue.yaml, continuity-delta.yaml, remarkability.yaml, research-ledger.yaml, book-strategy.yaml, and series/story-threads.yaml.\n\n${eventRule(root, "book-plan", "book-planning")}`;
}
...
export function researchUpdatePrompt(root: string, scope: string): string {
  const project = readProject(root);
  const book = readBook(root);
  return `Use the novel-forge-for-pi skill.\n\nProject root: ${root}\nActive book: ${book.book_id}\nResearch scope: ${scope}\n\nUpdate only evidence that the writer supplied, that exists in project files, or that is supported by identified sources. Keep author-taste evidence, public market observations, project reader evidence, and story-world research separate. Do not scrape retailers, invent reviews, simulate readers, or turn named-author references into imitation instructions. Unknown fields remain empty. This event must not advance the stage or change a pending creative gate.\n\nAllowed evidence includes taste-profile.yaml, voice-guardrails.yaml, bounded VE-NNN voice-experiment files, research-ledger.yaml, book-strategy.yaml, voice-audits.yaml, and research/source-register.yaml.\n\n${eventRule(root, "research-update", project.current_stage, `Pass scope: ${scope}. Submit at least one complete allowlisted evidence artifact.`)}`;
}
