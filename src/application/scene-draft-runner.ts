...
export async function runSceneDraftJob(input: RunSceneDraftJobInput): Promise<RunSceneDraftJobResult> {
  ...
  if (!modelExecutionProfileIdsMatch(modelProfile.id, input.capsule.model_execution_profile)) throw new Error(`Model profile ${modelProfile.id} does not match capsule profile ${input.capsule.model_execution_profile}.`);
  ...
  const tokenCounts = assertModelJobFits({ root: input.root, runId: input.runId, instruction: prompt, evidence: context, profile: modelProfile, jobType: "draft-scene" });
  ...
  const tokenCalibration = recordModelTokenCalibration({ root: input.root, runId: input.runId, callId, profile: modelProfile, counts: tokenCounts, ...(actualInputTokens !== undefined ? { actualInputTokens } : {}) });
