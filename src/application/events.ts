...
function renderEventStatus(project: ProjectState, book: BookState): string { return ["# Novel Forge Status", "", `- Project: ${project.project_name}`, `- Type: ${project.project_type}`, `- Profile: ${book.profile}`, `- Active book: ${book.book_id}`, `- Stage: ${project.current_stage}`, `- Next gate: ${project.next_gate ?? "none"}${project.next_gate ? ` (${project.gates[project.next_gate] ?? "unknown"})` : ""}`, `- Current chapter: ${book.current_chapter}`, `- Manuscript words: ${book.actual_words}`, "", "This snapshot was committed with the most recent guarded workflow event. Run `/novel-status` for a full integrity report.", ""].join("\n"); }
export function applyNovelEvent(root: string, input: NovelEventInput): NovelEventResult {
  const project = structuredClone(readProject(root)); const book = structuredClone(readBook(root)); validateFiles(root, input, project, book); const changes = input.files.map((file) => ({ path: normalized(file.path), content: file.content })); let queue: ChapterQueueState | null = null; let plot: PlotGridState | null = null; if (["book-plan", "chapter-queue", "draft-chapter"].includes(input.eventType)) ({ queue, plot } = validateArchitecture(root, changes, book, input.eventType, input.chapter));
  ...
  setChange(changes, "PROJECT.yaml", stringifyYaml(project)); setChange(changes, `books/${book.book_id}/BOOK.yaml`, stringifyYaml(book)); setChange(changes, "STATUS.md", renderEventStatus(project, book));
  const transaction = applyTransaction(root, changes, { gitCheckpoint: false }); const git = project.automation.git_checkpoints ? commitWorkflowEvent(root, transaction.changed, `Novel Forge: ${input.eventType}${input.chapter ? ` chapter-${input.chapter}` : ""}`) : null; return { changed: transaction.changed, stage: project.current_stage, projectHash: projectStateHash(root), gitMessage: git?.message ?? "Git checkpoints disabled." };
}
