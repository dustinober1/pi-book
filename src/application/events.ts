...
function allowedPath(event: NovelEventType, path: string, bookId: string, chapter?: number): boolean {
  const book = `books/${bookId}`; const exact: Record<NovelEventType, string[]> = {
    "voice-profile": ["series/voice-profile.md"], "series-plan": ["series/series-bible.md", "series/series-arc.yaml", "series/canon.yaml", "series/story-threads.yaml"],
    "book-plan": [`${book}/book-bible.md`, `${book}/genre.yaml`, `${book}/plot-grid.yaml`, `${book}/chapter-queue.yaml`, `${book}/continuity-delta.yaml`, "series/story-threads.yaml"],
    ...
    "canon-lock": ["series/canon.yaml", "series/story-threads.yaml", "series/series-arc.yaml"], package: [`${book}/package.md`],
  };
  ...
  if (["draft-chapter", "revise"].includes(event) && path.startsWith(`${book}/manuscript/chapters/`) && /\.md$/i.test(path)) {
    ...
    const match = basename(path).match(/^0*(\d+)(?:[-_ .]|$)/); return Boolean(match && Number.parseInt(match[1] ?? "", 10) === chapter);
  }
  return false;
}
...
function validateFiles(root: string, input: NovelEventInput, project: ProjectState, book: BookState): void {
  ...
  for (const file of input.files) { file.path = normalized(file.path); if (seen.has(file.path)) throw new Error(`Duplicate event path: ${file.path}`); seen.add(file.path); if (!allowedPath(input.eventType, file.path, book.book_id, input.chapter)) throw new Error(`${file.path} is not allowed for ${input.eventType}.`); }
  const required: Partial<Record<NovelEventType, RegExp>> = { "voice-profile": /series\/voice-profile\.md$/, "series-plan": /series\/(series-bible\.md|series-arc\.yaml)$/, "book-plan": /book-bible\.md$|plot-grid\.yaml$/, "chapter-queue": /chapter-queue\.yaml$/, "draft-chapter": /manuscript\/chapters\/.*\.md$/, review: /review-report\.md$|revision-tickets\.yaml$/, "canon-lock": /series\/canon\.yaml$/, package: /package\.md$/ };
  const pattern = required[input.eventType]; if (pattern && !input.files.some((file) => pattern.test(file.path))) throw new Error(`${input.eventType} event is missing its required output file.`);
}
...
function projectedWordCount(root: string, bookId: string, changes: FileChange[]): number {
  const rootPath = join(root, "books", bookId); const content = new Map<number, string>();
  for (const path of listChapterFiles(rootPath)) { const number = chapterNumber(path); if (number !== null) content.set(number, readText(path) ?? ""); }
  for (const change of changes) if (change.path.startsWith(`books/${bookId}/manuscript/chapters/`)) { const number = chapterNumber(change.path); if (number !== null) content.set(number, change.content); }
  return [...content.values()].reduce((sum, text) => sum + countWords(text), 0);
}
function validateArchitecture(root: string, files: FileChange[], book: BookState, event: NovelEventType, chapter?: number): { queue: ChapterQueueState; plot: PlotGridState } {
  const bookRoot = `books/${book.book_id}`; const profile = getProfile(book.profile);
  const genre = parseOverlay<GenreConfig>(root, files, `${bookRoot}/genre.yaml`, GenreConfigSchema); const plot = parseOverlay<PlotGridState>(root, files, `${bookRoot}/plot-grid.yaml`, PlotGridSchema); const queue = parseOverlay<ChapterQueueState>(root, files, `${bookRoot}/chapter-queue.yaml`, ChapterQueueSchema);
  ...
  const blockers = findings.filter((finding) => finding.severity === "blocker"); if (blockers.length) throw new Error(`Profile validation blocked ${event}:\n${blockers.map((item) => `- ${item.message}`).join("\n")}`);
  if (event === "draft-chapter") {
    const packet = queue.packets.find((item) => item.chapter === chapter); if (!packet || packet.status !== "ready") throw new Error(`Chapter ${chapter ?? "unknown"} packet is not ready.`);
    const canon = parseOverlay<CanonState>(root, files, "series/canon.yaml", CanonSchema); const threads = parseOverlay<StoryThreadsState>(root, files, "series/story-threads.yaml", StoryThreadsSchema); const sources = parseOverlay<SourceRegisterState>(root, files, "research/source-register.yaml", SourceRegisterSchema);
    const refs = packetReferenceFindings(packet, canon, threads, sources, plot).filter((finding) => finding.severity === "blocker"); if (refs.length) throw new Error(`Reference validation blocked drafting:\n${refs.map((item) => `- ${item.message}`).join("\n")}`);
  }
  return { queue, plot };
}

export function applyNovelEvent(root: string, input: NovelEventInput): NovelEventResult {
  const project = structuredClone(readProject(root)); const book = structuredClone(readBook(root)); validateFiles(root, input, project, book); const changes = input.files.map((file) => ({ path: normalized(file.path), content: file.content }));
  let queue: ChapterQueueState | null = null; let plot: PlotGridState | null = null;
  ...
  switch (input.eventType) {
    case "voice-profile": project.gates["voice-approval"] = "pending"; project.next_gate = "voice-approval"; break;
    case "series-plan": project.current_stage = "book-planning"; project.next_gate = null; break;
    case "book-plan": project.gates["book-plan-approval"] = "pending"; project.next_gate = "book-plan-approval"; break;
    case "chapter-queue": project.current_stage = "drafting"; project.next_gate = null; book.status = "drafting"; break;
    case "draft-chapter": {
      if (!input.chapter || !queue || !plot) throw new Error("draft-chapter requires a chapter number and valid queue."); const packet = queue.packets.find((item) => item.chapter === input.chapter); if (!packet) throw new Error(`Chapter ${input.chapter} packet not found.`); packet.status = "drafted"; setChange(changes, `books/${book.book_id}/chapter-queue.yaml`, stringifyYaml(queue));
      book.current_chapter = Math.max(book.current_chapter, input.chapter); book.actual_words = projectedWordCount(root, book.book_id, changes); book.status = "drafting";
      if (input.chapter === 1 && project.automation.require_first_chapter_approval && project.gates["first-chapter-approval"] !== "approved") { project.gates["first-chapter-approval"] = "pending"; project.next_gate = "first-chapter-approval"; project.current_stage = "drafting"; }
      else if (packet.milestone_gate) { if (!(packet.milestone_gate in project.gates)) throw new Error(`Unknown milestone gate: ${packet.milestone_gate}`); project.gates[packet.milestone_gate] = "pending"; project.next_gate = packet.milestone_gate; project.current_stage = "act-review"; book.act_checkpoint = packet.milestone_gate; }
    ...
    case "review": {
      const tickets = parseOverlay<RevisionTicketsState>(root, changes, `books/${book.book_id}/revision-tickets.yaml`, RevisionTicketsSchema); book.status = "review";
      if (openBlockingTickets(tickets).length) project.current_stage = "revision";
      else if (input.scope === "manuscript" || project.current_stage === "manuscript-review") { project.current_stage = "manuscript-review"; project.gates["manuscript-approval"] = "pending"; project.next_gate = "manuscript-approval"; }
      else { project.current_stage = "act-review"; if (!project.next_gate) { project.next_gate = "act-1-review"; project.gates["act-1-review"] = "pending"; } }
      break;
    }
  ...
  setChange(changes, "PROJECT.yaml", stringifyYaml(project)); setChange(changes, `books/${book.book_id}/BOOK.yaml`, stringifyYaml(book));
  const transaction = applyTransaction(root, changes, { gitCheckpoint: false }); refreshStatus(root); const changed = [...new Set([...transaction.changed, "STATUS.md"])];
  const git = project.automation.git_checkpoints ? commitWorkflowEvent(root, changed, `Novel Forge: ${input.eventType}${input.chapter ? ` chapter-${input.chapter}` : ""}`) : null;
  return { changed, stage: project.current_stage, projectHash: projectStateHash(root), gitMessage: git?.message ?? "Git checkpoints disabled." };
}
