...
export interface AcceptSceneCandidateResult {
  artifact: SceneAcceptanceArtifact;
  artifactPath: string;
...
function orderedSceneIndex(input: AcceptSceneCandidateInput, state: ChapterExecutionState): number {
  ...
  if (!Number.isInteger(input.stateDeltaExtractionAttempt) || input.stateDeltaExtractionAttempt < 1) {
    throw new Error("Scene acceptance requires a positive state-delta extraction attempt.");
  }
  ...
  if (state.accepted_scene_ids.length !== expectedAccepted.length
    || expectedAccepted.some((sceneId, position) => state.accepted_scene_ids[position] !== sceneId)) {
...
function requireDraft(input: AcceptSceneCandidateInput, state: ChapterExecutionState): SceneDraftArtifact {
  ...
  if (draft.run_id !== input.runId
    || draft.chapter !== state.chapter
    || draft.scene_id !== input.sceneId
    || draft.contract_hash !== state.contract_hash
    || draft.story_index_hash !== state.canon_snapshot_hash
    || draft.attempt !== input.draftAttempt) {
    throw new Error("Scene acceptance draft provenance does not match the execution checkpoint.");
  }
  if (hashText(draft.prose) !== draft.output_hash || draft.usage.outputHash !== draft.output_hash) {
    throw new Error("Scene acceptance draft output integrity check failed.");
  }
...
export function acceptSceneCandidate(input: AcceptSceneCandidateInput): AcceptSceneCandidateResult {
  ...
  if (!validation
    || !validation.passed
    || validation.next_node !== "critic-review"
    || validation.draft_output_hash !== draft.output_hash
    || validation.contract_hash !== draft.contract_hash) {
    throw new Error("Scene acceptance requires a passed deterministic validation artifact for the same draft.");
  }

  const criticSummary = readSceneCriticSummaryArtifact(input.root, input.runId, input.sceneId, input.draftAttempt);
  if (!criticSummary
    || !criticSummary.passed
    || criticSummary.next_action !== "state-delta"
    || criticSummary.draft_output_hash !== draft.output_hash
    || criticSummary.contract_hash !== draft.contract_hash) {
    throw new Error("Scene acceptance requires a passed critic summary for the same draft.");
  }

  const stateDelta = readSceneStateDeltaArtifact(
    input.root,
    input.runId,
    input.sceneId,
    input.draftAttempt,
  ...
  if (!stateDelta
    || !stateDelta.matches_expected
    || stateDelta.next_action !== "scene-accept"
    || stateDelta.draft_output_hash !== draft.output_hash
    || stateDelta.contract_hash !== draft.contract_hash) {
    throw new Error("Scene acceptance requires a matching state delta that is ready for scene-accept.");
  }

  if (readSceneAcceptanceArtifact(input.root, input.runId, input.sceneId, input.draftAttempt)) {
    throw new Error(`Scene acceptance artifact already exists for ${input.sceneId} attempt ${input.draftAttempt}.`);
  }
  ...
  const artifact: SceneAcceptanceArtifact = {
    ...
    draft_capsule_id: draft.capsule_id,
    contract_hash: draft.contract_hash,
    story_index_hash: draft.story_index_hash,
    validation_artifact_hash: artifactHash(validation),
    critic_summary_artifact_hash: artifactHash(criticSummary),
    state_delta_artifact_hash: artifactHash(stateDelta),
    state_delta_extraction_attempt: input.stateDeltaExtractionAttempt,
    accepted_prose: draft.prose,
    word_count: draft.word_count,
    accepted_mutations: stateDelta.actual_mutations,
    next_node: nextNode,
    next_scene_id: nextSceneId,
    accepted_at: timestamp(input.now),
  };
  ...
  const advanced: ChapterExecutionState = nextSceneId
    ? { ...transitioned, current_scene_id: nextSceneId, updated_at: timestamp(input.now) }
    : transitioned;
