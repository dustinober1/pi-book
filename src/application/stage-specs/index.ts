...
export function draftStageSpec(input: DraftStageInput): StageSpec {
  return {
    ...
    inputs: [input.contextText, `Context report: estimated ${input.estimatedTokens} tokens; excluded ${input.excluded.join(", ")}.`],
    must: ["Make the prose specific to the approved voice, characters, pressure, omissions, scene, and compact remarkability contract.", "Include the complete chapter Markdown plus any justified continuity, story-thread, or ticket deltas."],
