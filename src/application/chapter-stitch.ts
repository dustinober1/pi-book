...
export interface StitchAcceptedChapterResult {
  artifact: ChapterStitchArtifact;
  artifactPath: string;
...
function validateSceneOrder(input: StitchAcceptedChapterInput, state: ChapterExecutionState): void {
  ...
  if (state.accepted_scene_ids.length !== input.chapterSceneIds.length
    || input.chapterSceneIds.some((sceneId, index) => state.accepted_scene_ids[index] !== sceneId)) {
    throw new Error(`Accepted scene order does not match chapter scene order. Expected ${input.chapterSceneIds.join(", ")}; found ${state.accepted_scene_ids.join(", ")}.`);
  }
  if (state.current_scene_id !== input.chapterSceneIds.at(-1)) {
    throw new Error(`Chapter stitch checkpoint must remain on final scene ${input.chapterSceneIds.at(-1)}.`);
  }
}

function requireAcceptance(
  input: StitchAcceptedChapterInput,
  state: ChapterExecutionState,
  sceneId: string,
  index: number,
): SceneAcceptanceArtifact {
  ...
  if (acceptance.run_id !== input.runId
    || acceptance.chapter !== state.chapter
    || acceptance.scene_id !== sceneId
    || acceptance.draft_attempt !== attempt
    || acceptance.contract_hash !== state.contract_hash
    || acceptance.story_index_hash !== state.canon_snapshot_hash) {
    throw new Error(`Scene acceptance artifact provenance does not match chapter stitch for ${sceneId}.`);
  }
  if (hashText(acceptance.accepted_prose) !== acceptance.draft_output_hash) {
    throw new Error(`Scene acceptance prose hash does not match for ${sceneId}.`);
  }
  if (acceptance.word_count !== countWords(acceptance.accepted_prose)) {
    throw new Error(`Scene acceptance word count does not match for ${sceneId}.`);
  }
  ...
  if (acceptance.next_scene_id !== expectedNext || acceptance.next_node !== expectedNode) {
    throw new Error(`Scene acceptance routing provenance for ${sceneId} must point to ${expectedNext ?? "chapter-stitch"}.`);
  }
  return acceptance;
}
...
export function stitchAcceptedChapter(input: StitchAcceptedChapterInput): StitchAcceptedChapterResult {
  ...
  if (readChapterStitchArtifact(input.root, input.runId, state.chapter)) {
    throw new Error(`Chapter stitch artifact already exists for chapter ${state.chapter}.`);
  }
  ...
  const scenes: ChapterStitchScene[] = acceptances.map((item) => ({
    scene_id: item.scene_id,
    draft_attempt: item.draft_attempt,
    draft_output_hash: item.draft_output_hash,
    acceptance_artifact_hash: artifactHash(item),
  ...
  const artifact: ChapterStitchArtifact = {
    ...
    story_index_hash: state.canon_snapshot_hash,
    scene_ids: [...input.chapterSceneIds],
    scenes,
