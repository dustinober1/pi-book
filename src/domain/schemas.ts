...
export const ProjectTypeSchema = Type.Union([
  ...
  Type.Literal("planned-series"),
  Type.Literal("open-ended-series"),
]);
export type ProjectType = Static<typeof ProjectTypeSchema>;

export const StageSchema = Type.Union([
  Type.Literal("voice-intake"),
  Type.Literal("series-planning"),
  Type.Literal("book-planning"),
  Type.Literal("chapter-queue"),
  Type.Literal("drafting"),
  Type.Literal("act-review"),
  Type.Literal("revision"),
  Type.Literal("manuscript-review"),
  Type.Literal("canon-lock"),
...
export const CanonSchema = Type.Object({
  ...
  relationships: Type.Array(Type.Object({
    id: Type.String(),
    characters: Type.Array(Type.String(), { minItems: 2 }),
    state: Type.String(),
    trust: Type.String(),
    public_status: Type.String(),
    private_status: Type.String(),
...
export const StoryThreadsSchema = Type.Object({
  schema_version: Type.Literal("1.0.0"),
  threads: Type.Array(StoryThreadSchema),
});
...
export const ChapterPacketSchema = Type.Object({
  ...
  pov: Type.String(),
  purpose: Type.String(),
  scene_engine: Type.String(),
  pressure_movement: Type.String(),
  character_movement: Type.String(),
  relationship_movement: Type.String(),
  story_thread_refs: Type.Array(Type.String()),
  continuity_refs: Type.Array(Type.String()),
  character_refs: Type.Array(Type.String()),
  required_research: Type.Array(Type.String()),
  profile_fields: Type.Record(Type.String(), Type.Unknown()),
  ending_hook: Type.String(),
  milestone_gate: Type.Union([Type.String(), Type.Null()]),
  target_words: Type.Integer({ minimum: 300 }),
});
export type ChapterPacket = Static<typeof ChapterPacketSchema>;

export const ChapterQueueSchema = Type.Object({
  schema_version: Type.Literal("1.0.0"),
  active_window: Type.String(),
  packets: Type.Array(ChapterPacketSchema),
});
export type ChapterQueueState = Static<typeof ChapterQueueSchema>;

export const PlotGridSchema = Type.Object({
  schema_version: Type.Literal("1.0.0"),
  acts: Type.Array(Type.Object({
    id: Type.String(),
    purpose: Type.String(),
    start_chapter: Type.Integer({ minimum: 1 }),
    end_chapter: Type.Integer({ minimum: 1 }),
    gate: Type.Union([Type.String(), Type.Null()]),
  })),
  chapters: Type.Array(Type.Object({
    ...
    setup_ids: Type.Array(Type.String()),
    payoff_ids: Type.Array(Type.String()),
    profile_obligations: Type.Array(Type.String()),
  })),
});
...
export const RevisionTicketSchema = Type.Object({
  ...
  chapter: Type.Union([Type.Integer({ minimum: 1 }), Type.Null()]),
...
export const RevisionTicketsSchema = Type.Object({
  schema_version: Type.Literal("1.0.0"),
  tickets: Type.Array(RevisionTicketSchema),
});
export type RevisionTicketsState = Static<typeof RevisionTicketsSchema>;

export const ContinuityDeltaSchema = Type.Object({
  schema_version: Type.Literal("1.0.0"),
  proposed_facts: Type.Array(CanonFactSchema),
  conflicts: Type.Array(Type.Object({
    id: Type.String(),
    description: Type.String(),
    canon_refs: Type.Array(Type.String()),
...
export const GenreConfigSchema = Type.Object({
  ...
  requirements: Type.Record(Type.String(), Type.Unknown()),
});
export type GenreConfig = Static<typeof GenreConfigSchema>;

export const SeriesArcSchema = Type.Object({
  schema_version: Type.Literal("1.0.0"),
  books: Type.Array(Type.Object({
    id: Type.String({ pattern: "^book-[0-9]{2}$" }),
    status: Type.String(),
    role: Type.String(),
    closes: Type.Array(Type.String()),
    carries: Type.Array(Type.String()),
  })),
...
export const WorkflowSchema = Type.Object({
  schema_version: Type.Literal("1.0.0"),
  product: Type.Literal("novel-forge-for-pi"),
  stages: Type.Array(Type.Object({
    id: StageSchema,
    gate: Type.Union([Type.String(), Type.Null()]),
    next: Type.Union([StageSchema, Type.Null()]),
  })),
...
const schemaByPath: Array<[RegExp, TSchema]> = [
  ...
  [/(^|\/)BOOK\.yaml$/, BookSchema],
  [/(^|\/)canon\.yaml$/, CanonSchema],
  [/(^|\/)story-threads\.yaml$/, StoryThreadsSchema],
  [/(^|\/)plot-grid\.yaml$/, PlotGridSchema],
  [/(^|\/)chapter-queue\.yaml$/, ChapterQueueSchema],
  [/(^|\/)continuity-delta\.yaml$/, ContinuityDeltaSchema],
  [/(^|\/)revision-tickets\.yaml$/, RevisionTicketsSchema],
  [/(^|\/)genre\.yaml$/, GenreConfigSchema],
  [/(^|\/)series-arc\.yaml$/, SeriesArcSchema],
  [/(^|\/)source-register\.yaml$/, SourceRegisterSchema],
];

export function schemaForPath(path: string): TSchema | null {
  return schemaByPath.find(([pattern]) => pattern.test(path))?.[1] ?? null;
}
