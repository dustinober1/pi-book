...
export interface V1100ReleaseCheck {
  id: string;
  passed: boolean;
  detail: string;
}
...
function check(id: string, passed: boolean, detail: string): V1100ReleaseCheck {
  return { id, passed, detail };
}

export function verifyV1100ReleaseTree(root: string): V1100ReleaseCheck[] {
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  const checks = verifyV1100ReleaseTree(process.cwd());
