...
export interface V173ReleaseCheck {
  id: string;
  passed: boolean;
  detail: string;
}
...
function check(id: string, passed: boolean, detail: string): V173ReleaseCheck {
  return { id, passed, detail };
}

export function verifyV173ReleaseTree(root: string): V173ReleaseCheck[] {
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  const checks = verifyV173ReleaseTree(process.cwd());
