...
export interface V201ReleaseCheck {
  id: string;
  passed: boolean;
  detail: string;
}
...
function check(id: string, passed: boolean, detail: string): V201ReleaseCheck {
  return { id, passed, detail };
}

export function verifyV201ReleaseTree(root: string): V201ReleaseCheck[] {
  ...
  const notes = text(root, "docs/releases/v2.0.1.md");
  const skill = text(root, "SKILL.md");

  return [
    check("package-version", packageJson.version === "2.0.1", `package.json version is ${packageJson.version}.`),
    check("lock-version", lock.version === "2.0.1" && lock.packages[""]?.version === "2.0.1", `Lock versions are ${lock.version} and ${lock.packages[""]?.version ?? "missing"}.`),
    check("runtime-version", /NOVEL_FORGE_VERSION\s*=\s*"2\.0\.1"/.test(versionSource), "Runtime version constant reports 2.0.1."),
    check("release-script", packageJson.scripts["verify:release"] === "node --import tsx scripts/verify-v2-0-1-release.ts", "verify:release targets the v2.0.1 checker."),
    check("release-files", ["docs/releases/v1.10.0.md", "scripts/verify-v2-0-1-release.ts", "tests/v1-10-0-release-checklist.test.ts"].every((path) => existsSync(join(root, path))), "All v2.0.1 release files exist."),
    check("pinned-install", /pi install git:github\.com\/dustinober1\/pi-book@v2\.0\.1/.test(readme) && /@v2\.0\.1/.test(release), "README and release status pin v2.0.1."),
    check("changelog", /## 2\.0\.1 — Novel-Start Project Discovery Fix/.test(changelog), "Changelog contains the v2.0.1 heading."),
    check("release-notes", /No Novel Forge project found/.test(notes) && /requireProjectRoot/.test(notes), "Release notes describe the project-discovery fix."),
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  const checks = verifyV201ReleaseTree(process.cwd());
