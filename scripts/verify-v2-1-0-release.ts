...
export interface V210ReleaseCheck {
  id: string;
  passed: boolean;
  detail: string;
}
...
function check(id: string, passed: boolean, detail: string): V210ReleaseCheck {
  return { id, passed, detail };
}

export function verifyV210ReleaseTree(root: string): V210ReleaseCheck[] {
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  const checks = verifyV210ReleaseTree(process.cwd());
