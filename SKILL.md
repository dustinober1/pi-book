...
Every `source_ids` entry on a chronology, constraint, or knowledge-boundary item must be the `id` of a source actually registered in `research/source-register.yaml` — never a raw file path or the name of an existing project document such as `series/series-bible.md` or `series/canon.yaml`. If a chronology or constraint fact genuinely comes from that kind of in-project document, first add (or reuse) a source-register entry whose `location` names the document and gives it a stable `id` (for example `SRC-014`), submit `research/source-register.yaml` alongside `historical-context.yaml` in the same `book-plan` event, and reference that `id` — not the path — from `source_ids`. The validator rejects any `source_ids` value that does not resolve to a registered source, and a high-risk constraint additionally requires that at least one of its `research_ids` resolves to research that is `ready` with confidence above `low`.
...
Two of these semantic fields are not judgement at all. `start_state_ids` is "which established state records describe the people and things this chapter is about", and `knowledge_boundary_ids` is "what has this POV established knowledge of" — both are queries over `series/state-ledger.yaml` and `series/knowledge-ledger.yaml`, and the skeleton resolves them. Every ID returned names a record that already exists with an established status; a project with empty ledgers derives nothing and says so rather than guessing.

The remaining three are yours: `required_end_state` (what this chapter must have changed by its end), `forbidden_changes` (what it must not touch), and `scene_beats` (how the chapter divides into scenes). **Use `novel_complete_chapter_contract` rather than hand-authoring the contract file.** You pass those as typed values; the tool derives the rest, serialises the YAML itself — so a malformed scalar is impossible rather than merely discouraged — and applies the result through the same guarded `chapter-queue` event, which still validates schema, references, allowlists and story integrity. Every record ID you name must already exist in the ledgers; the tool rejects unknown IDs rather than inventing records, because typing the input must not move invention from malformed YAML into well-formed nonsense.

### Scene structure

`scene_beats` is an ordered list, one entry per scene, at most five. Each entry says three different things: `objective` is what the viewpoint is trying to do, `conflict` is what stops it, and `turn` is what has changed by the end. They must genuinely differ — an entry that names one fact three times is rejected, because those three fields are the entire brief a scene-drafting model receives.

Write them in the order the scenes happen. The chapter's ending hook belongs to its last scene; every earlier scene ends on its own turn. Leave `thread_ids` unset on a scene unless you mean that the chapter's other threads do not move in it — a scene with no active threads cannot report thread movement at all.

Scene structure is not derivable. A chapter packet's fields describe the whole chapter on several unlike axes — purpose, scene engine, pressure, character movement, relationship movement — and no arithmetic over them produces a scene sequence. A chapter longer than 1,000 words is therefore not executable until you supply `scene_beats`; a shorter one compiles to a single scene from the chapter's own named fields. Until then the chapter reports `scene_beats` in `missing_small_model_fields` and can only be drafted with a `draft-chapter` event, without scene critics, targeted repair, or ordered acceptance.
